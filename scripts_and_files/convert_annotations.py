                    ...
                    if filename.endswith(".txt"):
                        ...
                        with open(filename) as f:
                            for line in f:
                                ...
                                labels = line.split()
                                coords = np.asarray([float(labels[3]), float(labels[4]), float(labels[5]), float(labels[6])])
                                coords = convert(filename_str, coords)
                                labels[3], labels[4], labels[5], labels[6] = coords[0], coords[1], coords[2], coords[3]
                                newline = str(0) + " " + str(labels[3]) + " " + str(labels[4]) + " " + str(labels[5])+ " " + str(labels[6])
